...
echo 'Enter new hostname of the machine (e.g. macbook-ponti)'
  read hostname
...
if [[ `uname` == 'Darwin' ]]; then
  ...
  if [[ $? != 0 ]]; then
    echo 'Installing Homebrew...'
      ...
      brew update
      brew install mongo nginx nvm
  fi

  echo "Installing ZSH & oh-my-zsh"
    source 'etc/zsh-install.sh'

  echo "Install NodeJS"
    source 'etc/nodejs-install.sh'
...
open_apps() {
  ...
  echo 'iTerm2:'
  open http://iterm2.com/downloads.html
}
